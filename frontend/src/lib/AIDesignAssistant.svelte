<script>
  ...
  function openAssistant() {
    ...
    if (contextualCourseId) courseId = contextualCourseId;
  }

  function closeAssistant() { isOpen = false; }
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        courseId = courseFromHash() || courses[0]?.course_id || '';
      }
    } catch {
...
{#if isOpen}
  <div class="assistant-scrim" role="presentation" onclick={closeAssistant}></div>
  <aside class="assistant-panel" aria-label="AI Design Assistant">
    ...
    <section class="assistant-body">
      {#if mode !== 'course'}
        <label>Course context<select bind:value={courseId} onchange={() => (moduleId = '')}><option value="">Select a course</option>{#each courses as course}<option value={course.course_id}>{course.title}</option>{/each}</select></label>
      {/if}
